...
In one approach, using a syntax convention that $r_{n}^{+} \left[\left[ x_{n} \right]\right]$ means those expressions produced by applying the rules $r_{n}^{+}$ to a set of mathematical expressions $x_{n}$, and a syntax convention that $m_{n}^{++} \left[\left[ x_{n}, r_{n}^{+}, r_{n}^{-} \right]\right]$ means those rules produced by applying the rules $m_{n}^{++}$ to a set of mathematical expressions $x_{n}$ and sets of rules $r_{n}^{+}$ and $r_{n}^{-}$, the following could be stated with respect to a possible implementation of a function $C$:

$$ x_{n+1} = \left( x_{n} \cup r_{n}^{+} \left[\left[ x_{n} \right]\right] \right) \setminus r_{n}^{-} \left[\left[ x_{n} \right]\right] $$

$$ r_{n+1}^{+} = \left( r_{n}^{+} \cup m_{n}^{++} \left[\left[ x_{n}, r_{n}^{+}, r_{n}^{-} \right]\right] \right) \setminus m_{n}^{+-} \left[\left[ x_{n}, r_{n}^{+}, r_{n}^{-} \right]\right] $$
...
$$ r_{n+1}^{+} = \left( r_{n}^{+} \cup m_{0}^{++} \left[\left[ x_{n}, r_{n}^{+}, r_{n}^{-} \right]\right] \right) \setminus m_{0}^{+-} \left[\left[ x_{n}, r_{n}^{+}, r_{n}^{-} \right]\right] $$
